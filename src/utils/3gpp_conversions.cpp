...
void xgpp_conv::data_notification_from_openapi(
    const oai::amf_server::model::NotificationData& nd,
  ...
  std::shared_ptr<amf::nf_profile> p = {};

  // Only support UPF for now
  if (nd.getNfProfile().getNfType() == "UPF")
    p = std::make_shared<amf::upf_profile>();
...
void xgpp_conv::amf_event_exposure_notification_from_openapi(
    const oai::amf_server::model::NamfEventExposure& nee,
  ...
  // PDU session ID
  //if (nee.pduSeIdIsSet()) {
  //  Logger::amf_api_server().debug("PDU Session ID %d", nee.getPduSeId());
