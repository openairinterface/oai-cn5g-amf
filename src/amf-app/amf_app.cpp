...
bool amf_app::update_pdu_sessions_context(
  ...
  std::shared_ptr<ue_context> uc = get_ue_context(supi);
  if (!uc) return false;
  auto pdu_session_status =
      statusNotification.getStatusInfo().getResourceStatus().getEnumValue();
...
void amf_app::handle_itti_message(
  ...
  if (!amf_n2_inst->get_ngap_ctx_by_ran_amf(
...
void amf_app::handle_itti_message(itti_sbi_n1_message_notification& itti_msg) {
  ...
  std::shared_ptr<ue_ngap_context> unc = {};
  if (!amf_n2_inst->get_ngap_ctx_by_ran_amf(ran_ue_ngap_id, gnb_id, unc)) {
    Logger::amf_app().debug(
