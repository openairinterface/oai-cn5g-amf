...
//------------------------------------------------------------------------------
// TODO: UE-AMBR is not sourced from the UDM AM subscription data yet, so this
// always falls back to the compile-time constant.
static void resolve_ue_ambr(
  ...
  if (uc && uc->has_ue_ambr) {
    ...
    ul = uc->ue_ambr_ul;
    Logger::amf_n2().debug(
        "Using UE-AMBR from UE context (DL %lu, UL %lu bps)", dl, ul);
  } else {
    dl = UE_AGGREGATE_MAXIMUM_BIT_RATE_DL;
    ul = UE_AGGREGATE_MAXIMUM_BIT_RATE_UL;
    Logger::amf_n2().debug(
        "UE-AMBR not available in UE context; using compile-time constant "
        "(DL %lu, UL %lu bps)",
        dl, ul);
  }
}
...
void amf_n2::handle_itti_message(
  ...
  std::vector<uint8_t> buffer(BUFFER_SIZE_1024, 0);
  ...
  bstring b = blk2bstr(buffer.data(), encoded);
...
void amf_n2::handle_itti_message(
  ...
  itti_msg->ue_ctx_req = init_ue_msg->init_ue_message->getUeContextRequest();
  if (!itti_msg->ue_ctx_req) {
  ...
  if (unc->initial_ue_msg.buf) {
    ...
    std::vector<uint8_t> initial_ue_msg_buf(BUFFER_SIZE_1024, 0);

    int encoded_size = init_ue_msg->init_ue_message->Encode(
        initial_ue_msg_buf.data(), BUFFER_SIZE_1024);

    if (encoded_size > 0) {
      Logger::amf_n2().debug("Encoded InitialUEMessage size %d", encoded_size);
      memcpy(
          (void*) unc->initial_ue_msg.buf, initial_ue_msg_buf.data(),
          encoded_size);
...
void amf_n2::handle_itti_message(
  ...
  if (unc->amf_ue_ngap_id != amf_ue_ngap_id) {
    ...
    Logger::amf_n2().error(
        ...
        "); aborting Uplink NAS Transport",
        amf_ue_ngap_id, unc->amf_ue_ngap_id);
    std::optional<long> amf_id_opt =
        std::make_optional<long>(static_cast<long>(amf_ue_ngap_id));
    std::optional<uint32_t> ran_id_opt =
        std::make_optional<uint32_t>(ran_ue_ngap_id);

    // 5GS TMSI
    FiveGSTmsi tmp = {};
    tmp.set(unc->s_setid, unc->s_pointer, unc->s_tmsi);
    std::optional<FiveGSTmsi> five_gs_tmsi =
        std::make_optional<FiveGSTmsi>(tmp);

    send_ng_error_indication(
        ul_nas_transport->assoc_id, ul_nas_transport->stream, amf_id_opt,
        ran_id_opt, Ngap_Cause_PR_radioNetwork,
        Ngap_CauseRadioNetwork_inconsistent_remote_UE_NGAP_ID, five_gs_tmsi);
    return;
  }
  /*
...
void amf_n2::handle_itti_message(
  ...
  if ((encoded_size < 0) || (encoded_size > BUFFER_SIZE_1024)) {
    Logger::amf_n2().error("Encoding failed or buffer too small");
    return;
  }
  bstring b = blk2bstr(buffer, encoded_size);
...
void amf_n2::handle_itti_message(
  ...
  if (itti_msg->is_sr or !itti_msg->pdu_sessions.empty()) {
    ...
    if (unc->ue_radio_cap_ind) {
      // TODO: re-enable, the gNB otherwise has to run its own UE Capability
      // Enquiry (TS 38.413 §9.3.1.73).
    ...
    if (!itti_msg->pdu_sessions.empty()) {
      ...
      for (auto& p : itti_msg->pdu_sessions) {
        ...
        // TODO: attach item.nasPdu here (TS 38.413 §8.2.1.2).
      ...
      // TODO: add Mobility Restriction List (TS 38.413 §9.3.1.52).
    }
  }

  uint8_t buffer[BUFFER_SIZE_2048];
  int encoded_size = msg->Encode(buffer, BUFFER_SIZE_2048);
  if ((encoded_size < 0) || (encoded_size > BUFFER_SIZE_2048)) {
    Logger::amf_n2().error("Encoding failed or buffer too small");
    return;
  }
  bstring b = blk2bstr(buffer, encoded_size);
...
void amf_n2::handle_itti_message(
  ...
  if (!itti_msg->pdu_sessions.empty()) {
    for (auto& p : itti_msg->pdu_sessions) {
      ...
      std::shared_ptr<pdu_session_context> psc = {};
      if (!amf_app_inst->get_pdu_session_context(nc->supi, p.first, psc)) {
        // TODO: use the NSSAI from the N1N2MessageTransfer request instead of
        // the default (TS 38.413 §9.3.1.50).
        Logger::amf_n2().debug(
  ...
  if (encoded_size < 0) {
    Logger::amf_n2().error("Encoding failed");
    if (buffer) free(buffer);
    for (auto& item : list) {
      oai::utils::utils::bdestroy_wrapper(&item.nasPdu);
    }
    return;
  }
...
void amf_n2::handle_itti_message(
  ...
  // TODO: the optional UE-AMBR IE is not set here, PduSessionResourceModify-
  // RequestMsg has no setter for it yet (TS 38.413 §9.3.1.47).
...
void amf_n2::handle_itti_message(
  ...
  if ((encoded_size < 0) || (encoded_size > BUFFER_SIZE_256)) {
    Logger::amf_n2().error("Encoding failed or buffer too small");
    return;
  }
...
void amf_n2::handle_itti_message(
  ...
  Logger::amf_n2().debug(
      "UE Context Release Complete ran_ue_ngap_id (" RAN_UE_NGAP_ID_FMT
  ...
  if (nc != nullptr) {
    ...
    uint64_t current_amf_ue_ngap_id = nc->amf_ue_ngap_id;
    if (current_amf_ue_ngap_id != amf_ue_ngap_id) {
      ...
      Logger::amf_n2().debug(
          ...
          ", received amf_ue_ngap_id " AMF_UE_NGAP_ID_FMT ")",
          current_amf_ue_ngap_id, amf_ue_ngap_id);
      remove_amf_ue_ngap_id_2_ue_ngap_context(amf_ue_ngap_id);
      remove_ran_ue_ngap_id_2_ngap_context(ran_ue_ngap_id, gc->gnb_id);
      return;
    } else {
...
*/
  ...
  for (auto pdu_session : pdu_sessions_to_be_released) {
    ...
    smf_responses.emplace(pdu_session.pduSessionId, f);
  ...
  while (!smf_responses.empty()) {
    ...
    if (result_opt.has_value()) {
      ...
      Logger::amf_n2().debug(
          "Got result for PDU Session Id %d", smf_responses.begin()->first);
      ...
      if (result_json.find(kSbiResponseHttpResponseCode) != result_json.end()) {
        ...
        Logger::amf_n2().warn("Couldn't get the HTTP response code");
      }
    } else {
      is_up_activated = false;
      Logger::amf_n2().warn("Couldn't get the HTTP response");
    }
  ...
  // Remove the UE NGAPcontext only: the N2 signalling connection
  // is gone, but the UE is still registered.
  remove_amf_ue_ngap_id_2_ue_ngap_context(amf_ue_ngap_id);
  remove_ran_ue_ngap_id_2_ngap_context(ran_ue_ngap_id, gc->gnb_id);

  Logger::amf_n2().debug(
      "Keeping amf_app UE context for amf_ue_ngap_id " AMF_UE_NGAP_ID_FMT
      " (UE is CM-IDLE but still registered)",
      amf_ue_ngap_id);
}
...
bool amf_n2::handle_itti_message(
  ...
  oai::utils::utils::bdestroy_wrapper(&knh_bs);
  Logger::amf_n2().debug(
  ...
  std::map<uint8_t, boost::shared_future<nlohmann::json>> smf_responses;
  ...
  for (auto& item : item_ho_required_list) {
    ...
    Logger::amf_n2().debug("PDU Session ID %d", pdu_session_id_value);
    std::shared_ptr<pdu_session_context> psc = {};
    if (amf_app_inst->get_pdu_session_context(
      ...
      // Store the future to be processed later
      amf_app_inst->store_promise(promise_id, p);
      smf_responses.emplace(psc->pdu_session_id, f);
      Logger::amf_n2().debug("Promise ID generated %d", promise_id);
  ...
  auto free_list_bufs = [&list]() {
    for (auto& item : list) {
      if (item.pduSessionResourceSetupRequestTransfer.buf) {
        free(item.pduSessionResourceSetupRequestTransfer.buf);
        item.pduSessionResourceSetupRequestTransfer.buf  = nullptr;
        item.pduSessionResourceSetupRequestTransfer.size = 0;
      }
    }
  };
  ...
  while (!smf_responses.empty()) {
    ...
    oai::utils::utils::wait_for_result(
        smf_responses.begin()->second, result_json_opt);
    if (result_json_opt.has_value()) {
      ...
      if (result_json.find("n2sm") != result_json.end()) {
        ...
        std::shared_ptr<pdu_session_context> psc = {};
        if (amf_app_inst->get_pdu_session_context(
                nc->supi, smf_responses.begin()->first, psc)) {
          PDUSessionResourceSetupRequestItem_t item = {};
  ...
  if ((encoded_size < 0) || (encoded_size > BUFFER_SIZE_8192)) {
    Logger::amf_n2().error("Encoding failed or buffer too small");
    free_list_bufs();
    return false;
  }
...
void amf_n2::handle_itti_message(
  ...
  Logger::amf_n2().debug(
      "Handover Request Ack ran_ue_ngap_id (" RAN_UE_NGAP_ID_FMT
      ") amf_ue_ngap_id (" AMF_UE_NGAP_ID_FMT ")",
      ran_ue_ngap_id, amf_ue_ngap_id);
  ...
  for (auto pdu_session_resource : list) {
    ...
    amf_app_inst->store_promise(promise_id, p);
    Logger::amf_n2().debug("Promise ID generated %d", promise_id);
    smf_responses.emplace(pdu_session_resource.pduSessionId, f);
  ...
  while (!smf_responses.empty()) {
    ...
    if (result_json_opt.has_value()) {
      ...
      if (result_json.find("n2sm") != result_json.end()) {
        ...
        result                                 = result && true;
        uint8_t pdu_session_id_value           = smf_responses.begin()->first;
        unsigned int data_len                  = n2_sm.length();
        PduSessionId pdu_session_id            = {};
        ...
        if (handoverCommandTransfer.buf) {
          ...
          handoverCommandTransfer.buf  = nullptr;
          handoverCommandTransfer.size = 0;
        }
...
void amf_n2::handle_itti_message(
  ...
  Logger::amf_n2().debug(
      "Handover Notify ran_ue_ngap_id (" RAN_UE_NGAP_ID_FMT
      ") amf_ue_ngap_id (" AMF_UE_NGAP_ID_FMT ")",
      ran_ue_ngap_id, amf_ue_ngap_id);
  ...
  std::shared_ptr<ue_context> uc =
      amf_app_inst->get_ue_context(old_ran_ue_ngap_id, amf_ue_ngap_id);

  if (uc == nullptr) {
    Logger::amf_n2().warn(
        "No existing UE context with amf_ue_ngap_id " AMF_UE_NGAP_ID_FMT
        ", ran_ue_ngap_id " RAN_UE_NGAP_ID_FMT ", ignore Handover Notify",
  ...
  for (auto pdu_session : sessions_ctx) {
    if (pdu_session->is_ho_accepted) {
      ...
      amf_app_inst->store_promise(promise_id, p);
      Logger::amf_n2().debug("Promise ID generated %d", promise_id);
      smf_responses.emplace(pdu_session->pdu_session_id, f);
  ...
  Logger::amf_n2().info("Send UE Release Command to source gNB");
...
void amf_n2::handle_itti_message(
  ...
  if (drb_subject_item_list.size() > 0)
  ...
  uint8_t buffer[BUFFER_SIZE_1024];
  int encoded_size = dl_ran_status_transfer->Encode(buffer, BUFFER_SIZE_1024);
  if ((encoded_size < 0) || (encoded_size > BUFFER_SIZE_1024)) {
    Logger::amf_n2().error("Encoding failed or buffer too small");
    return;
  }
...
void amf_n2::send_handover_preparation_failure(
  ...
  ho_preparation_failure_msg->setRanUeNgapId(ran_ue_ngap_id);
...
//------------------------------------------------------------------------------
void amf_n2::send_ng_error_indication(
    ...
    const std::optional<FiveGSTmsi>& tmsi) {
  auto error_indication_msg = std::make_unique<ErrorIndication>();

  if (amf_ue_ngap_id.has_value())
    error_indication_msg->setAmfUeNgapId(amf_ue_ngap_id.value());
  if (ran_ue_ngap_id.has_value())
    error_indication_msg->setRanUeNgapId(ran_ue_ngap_id.value());
  error_indication_msg->setCause(cause_present, cause_value);
  if (tmsi.has_value()) {
    error_indication_msg->setFiveGSTmsi(tmsi.value());
  }

  uint8_t buffer[BUFFER_SIZE_1024];
  int encoded_size = error_indication_msg->Encode(buffer, BUFFER_SIZE_1024);
  ...
  Logger::amf_n2().debug(
      "Sending NGAP Error Indication to gNB (assoc_id %d)", assoc_id);
  bstring b = blk2bstr(buffer, encoded_size);
  sctp_s_38412.sctp_send_msg(assoc_id, stream, &b);
  oai::utils::utils::bdestroy_wrapper(&b);
}
...
void amf_n2::send_ng_setup_failure(
  ...
  int encoded = ngSetupFailure.Encode(buffer.data(), BUFFER_SIZE_1024);
...
bool amf_n2::ran_ue_id_2_ue_ngap_context(
  ...
  if (uc != nullptr && uc->get_ngap_ctx() != nullptr) {
    unc = uc->get_ngap_ctx();
    return true;
  }
...
void amf_n2::get_ue_ngap_contexts(
  ...
  amf_app_inst->for_each_ue_context([&](const std::shared_ptr<ue_context>& uc) {
    ...
    if (!uc) return;
    std::shared_ptr<ue_ngap_context> unc = uc->get_ngap_ctx();
    if (unc == nullptr) return;
    if (unc->gnb_assoc_id == gnb_assoc_id) ue_contexts.push_back(unc);
  });
}
