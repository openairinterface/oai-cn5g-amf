...
amf_config::amf_config(
  ...
  add_nf(oai::config::PCF_CONFIG_NAME, m_pcf);
  ...
  try {
      ...
      if (list) {
        ...
        for (const auto& entry : list) {
          ...
          if (!std::regex_match(mcc, std::regex(MCC_REGEX)) ||
            ...
            throw std::invalid_argument(
                "Roaming partner MCC must have 3 decimal digits and MNC 2 or 3");
          }
    ...
    if (sepp) {
      ...
      if (!std::regex_match(host, std::regex("[A-Za-z0-9.-]+")) ||
          port == 0 || port > 65535 || (scheme != "http" && scheme != "https"))
