...
static void* data_decode_from_file(
    enum asn_transfer_syntax, asn_TYPE_descriptor_t* asnTypeOfPDU, FILE* file,
    const char* name, ssize_t suggested_bufsize, int first_pdu);
...
static void CC_PRINTFLIKE(1, 2) DEBUG(const char* fmt, ...) {
  va_list ap;
  if (!opt_debug) return;
  fprintf(stderr, "AD: ");
  va_start(ap, fmt);
...
static int has_codec_defined(
    const asn_TYPE_descriptor_t* td, const syntax_selector* element) {
  return *(const void* const*) (const void*) ((const char*) td->op + element->codec_offset) !=
         0;
}
 ...
 */
static const syntax_selector* ats_by_name(
    const char* name, const asn_TYPE_descriptor_t* td,
...
int main(int ac, char* av[]) {
  ...
  if (!anyPduType) {
    ...
    if (!anyPduType) {
      fprintf(
          stderr, "Empty PDU collection, no reference PDU to choose from.\n");
      exit(EX_SOFTWARE);
    }
#else
    ...
    switch (ch) {
      ...
      case 'o':
        ...
        if (sel) {
          ...
          fprintf(
              stderr, "-o<format>: '%s': improper format selector\n", optarg);
          exit(EX_UNAVAILABLE);
        }
        break;
      case '1':
      ...
      case 's':
        ...
        if (opt_stack < 0) {
          fprintf(stderr, "-s %s: Non-negative value expected\n", optarg);
          exit(EX_UNAVAILABLE);
        }
        break;
#ifdef JUNKTEST
      case 'J':
        ...
        if (opt_jprob <= 0.0 || opt_jprob > 1.0) {
          fprintf(stderr, "-J %s: Probability range 0..1 expected \n", optarg);
          exit(EX_UNAVAILABLE);
        }
        break;
      case 'R':
        ...
        if (random_max_size < 0) {
          fprintf(stderr, "-R %s: Non-negative value expected\n", optarg);
          exit(EX_UNAVAILABLE);
        }
        srandomdev();
        break;
...
#define _ASX(x) _AXS(x)
        ...
        for (sel = output_encodings; sel->name; sel++) {
          if (ats_by_name(sel->name, anyPduType, sel)) {
            fprintf(
                stderr, "  -o%s%s       %s%s\n", sel->name,
                strlen(sel->name) > 3 ? "" : " ", sel->full_name,
        ...
        fprintf(
            ...
            "  -R <size>    Generate a random value of roughly the given "
            "size,\n"
    ...
    for (ac_i = (isyntax == ATS_RANDOM) ? -1 : 0; ac_i < ac; ac_i++) {
      ...
      for (first_pdu = 1; (first_pdu || !opt_onepdu); first_pdu = 0) {
        ...
        if (osyntax == ATS_INVALID) {
          ...
          erv = asn_encode(
              NULL, osyntax, pduType, structure, write_out, binary_out);

          if (erv.encoded == -1) {
            fprintf(
                stderr, "%s: Cannot convert %s into %s\n", name, pduType->name,
          ...
          DEBUG(
              "Encoded in %" ASN_PRI_SSIZE " bytes of %s", erv.encoded,
              ats_simple_name(osyntax));
        }
...
static void buffer_dump() {
  ...
  if (!opt_debug) return;
  DEBUG(
      "Buffer: { d=%p, o=%" ASN_PRI_SIZE ", l=%" ASN_PRI_SIZE
      ", u=%" ASN_PRI_SIZE ", a=%" ASN_PRI_SIZE ", s=%" ASN_PRI_SIZE " }",
      (const void*) DynamicBuffer.data, DynamicBuffer.offset,
      DynamicBuffer.length, DynamicBuffer.unbits, DynamicBuffer.allocated,
      (size_t) DynamicBuffer.bytes_shifted);
  for (; p < e; p++) {
    fprintf(
        stderr, " %c%c%c%c%c%c%c%c", ((*p >> 7) & 1) ? '1' : '0',
        ((*p >> 6) & 1) ? '1' : '0', ((*p >> 5) & 1) ? '1' : '0',
        ((*p >> 4) & 1) ? '1' : '0', ((*p >> 3) & 1) ? '1' : '0',
        ((*p >> 2) & 1) ? '1' : '0', ((*p >> 1) & 1) ? '1' : '0',
        ((*p >> 0) & 1) ? '1' : '0');
  ...
  if (DynamicBuffer.unbits) {
    ...
    fprintf(
        stderr, " %" ASN_PRI_SSIZE ":%" ASN_PRI_SSIZE "\n",
        (ssize_t) DynamicBuffer.length - 1, (ssize_t) 8 - DynamicBuffer.unbits);
  } else {
...
static void buffer_shift_left(size_t offset, int bits) {
  ...
  DEBUG(
      "Shifting left %d bits off %ld (o=%ld, u=%ld, l=%ld)", bits,
  ...
  if (offset) {
    ...
    DEBUG(
        "oleft: %c%c%c%c%c%c%c%c", ((ptr[-1] >> 7) & 1) ? '1' : '0',
        ((ptr[-1] >> 6) & 1) ? '1' : '0', ((ptr[-1] >> 5) & 1) ? '1' : '0',
        ((ptr[-1] >> 4) & 1) ? '1' : '0', ((ptr[-1] >> 3) & 1) ? '1' : '0',
        ((ptr[-1] >> 2) & 1) ? '1' : '0', ((ptr[-1] >> 1) & 1) ? '1' : '0',
        ((ptr[-1] >> 0) & 1) ? '1' : '0');

    DEBUG(
        "oriht: %c%c%c%c%c%c%c%c", ((ptr[0] >> 7) & 1) ? '1' : '0',
        ((ptr[0] >> 6) & 1) ? '1' : '0', ((ptr[0] >> 5) & 1) ? '1' : '0',
        ((ptr[0] >> 4) & 1) ? '1' : '0', ((ptr[0] >> 3) & 1) ? '1' : '0',
        ((ptr[0] >> 2) & 1) ? '1' : '0', ((ptr[0] >> 1) & 1) ? '1' : '0',
        ((ptr[0] >> 0) & 1) ? '1' : '0');
    ...
    DEBUG(
        "after: %c%c%c%c%c%c%c%c", ((ptr[-1] >> 7) & 1) ? '1' : '0',
        ((ptr[-1] >> 6) & 1) ? '1' : '0', ((ptr[-1] >> 5) & 1) ? '1' : '0',
        ((ptr[-1] >> 4) & 1) ? '1' : '0', ((ptr[-1] >> 3) & 1) ? '1' : '0',
        ((ptr[-1] >> 2) & 1) ? '1' : '0', ((ptr[-1] >> 1) & 1) ? '1' : '0',
        ((ptr[-1] >> 0) & 1) ? '1' : '0');
  ...
  *ptr <<= bits;
  ...
  DEBUG(
      "Shifted. Now (o=%" ASN_PRI_SIZE ", u=%" ASN_PRI_SIZE " l=%" ASN_PRI_SIZE
      ")",
...
static void add_bytes_to_buffer(const void* data2add, size_t bytes) {
  ...
  DEBUG(
      "=> add_bytes(%" ASN_PRI_SIZE ") { o=%" ASN_PRI_SIZE " l=%" ASN_PRI_SIZE
      " u=%" ASN_PRI_SIZE ", s=%" ASN_PRI_SIZE " }",
      bytes, DynamicBuffer.offset, DynamicBuffer.length, DynamicBuffer.unbits,
  ...
  if (DynamicBuffer.allocated >=
    ...
    memmove(
        DynamicBuffer.data, DynamicBuffer.data + DynamicBuffer.offset,
        DynamicBuffer.length);
    ...
    if (DynamicBuffer.length) {
      memcpy(
          p, DynamicBuffer.data + DynamicBuffer.offset, DynamicBuffer.length);
    }
    ...
    DEBUG(
        "\tBuffer reallocated to %ld (%d time)", newsize,
        DynamicBuffer.nreallocs);
  }

  memcpy(
      DynamicBuffer.data + DynamicBuffer.offset + DynamicBuffer.length,
      data2add, bytes);
...
static int is_syntax_PER(enum asn_transfer_syntax syntax) {
  return (
      syntax == ATS_UNALIGNED_BASIC_PER ||
      syntax == ATS_UNALIGNED_CANONICAL_PER ||
      syntax == ATS_ALIGNED_BASIC_PER || syntax == ATS_ALIGNED_CANONICAL_PER);
}

static int restartability_supported(enum asn_transfer_syntax syntax) {
  return !is_syntax_PER(syntax);
}

static void* data_decode_from_file(
    enum asn_transfer_syntax isyntax, asn_TYPE_descriptor_t* pduType,
    FILE* file, const char* name, ssize_t suggested_bufsize, int on_first_pdu) {
  static uint8_t* fbuf;
  ...
  for (tolerate_eof = 1; /* Allow EOF first time buffer is non-empty */
    ...
    if (is_syntax_PER(isyntax) && opt_nopad) {
      ...
      if (isyntax == ATS_UNALIGNED_BASIC_PER)
        rval = uper_decode(
            opt_codec_ctx, pduType, (void**) &structure, i_bptr, i_size, 0,
      ...
      rval = asn_decode(
          opt_codec_ctx, isyntax, pduType, (void**) &structure, i_bptr, i_size);
    }
    ...
    DEBUG(
        "decode(%" ASN_PRI_SIZE ") consumed %" ASN_PRI_SIZE
    ...
    switch (rval.code) {
      ...
      case RC_WMORE:
        DEBUG(
            ...
            " with %ld..%ld-%ld..%ld",
            (long) rd, (long) rval.consumed, (long) DynamicBuffer.offset,
  ...
  if (on_first_pdu || DynamicBuffer.length ||
    ...
    DEBUG(
        "ofp %d, no=%ld, oo=%ld, dbl=%ld", on_first_pdu, (long) new_offset,
 ...
 */
static void junk_bytes_with_probability(
    uint8_t* buf, size_t size, double prob) {
  static int junkmode;
  uint8_t* ptr;
  uint8_t* end;
  ...
  for (ptr = buf, end = ptr + size; ptr < end; ptr++) {
    int byte = *ptr;
    if (junkmode++ & 1) {
      if ((((double) random() / RAND_MAX) < prob)) byte = random() & 0xff;
    } else {
