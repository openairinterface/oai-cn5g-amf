...
- **Email subject**: `[SEC-VUL-OAI-CN5G-AMF]: "Mention the affected
  functionality here"`.
...
in the
[GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-amf/issues),
...
- **Issue title**: `[AI-SEC-VUL-OAI-CN5G-AMF]: "Mention the affected
  functionality here"`.
- **Issue label**: apply the
  https://github.com/openairinterface/oai-cn5g-amf/labels/security
...
The [openairinterface/oai-cn5g-amf](https://github.com/openairinterface/oai-cn5g-amf)
repository contains the OAI implementation of the 5G Core **Access and Mobility
Management Function (AMF)**, together with its configuration templates,
container images, deployment artifacts, and CI tooling. Only the 5G Core is in
scope in this repository.

Issues in the other OAI CN5G network functions (SMF, UPF, AUSF, UDM, UDR, NRF,
...
can be reported here if they are reachable through the AMF. In that case, please
state clearly which component is affected and give the commit ID of the affected
submodule in addition to the AMF commit.

Security reports are in scope when they affect the confidentiality, integrity,
or availability of the AMF running in a documented or reasonably expected
deployment.

In-scope examples include:

- Memory corruption, crashes, infinite loops, resource exhaustion, or denial of
  service triggered by malformed or unexpected NAS (N1), NGAP/SCTP (N2), or SBI
  (HTTP/1.1, HTTP/2, JSON) messages, including ASN.1 decoding defects.
- Authentication, ciphering, integrity-protection, or key-derivation flaws in
  5G-AKA/SEAF handling, NAS security mode procedures, SUCI/SUPI de-concealment,
  or NH/NCC key chaining during N2 handover (3GPP TS 33.501).
- NAS or NGAP state-machine bypasses that allow an unauthenticated or
  unauthorised UE or gNB to register, obtain service, impersonate another
  subscriber, or take over an existing UE context.
- Exposure of subscriber identifiers (SUPI/IMSI/SUCI/GUTI), security keys,
  credentials, or user NAS content through logs, statistics, API responses, or
  error paths.
- Remote code execution, privilege escalation, arbitrary file access, or command
  execution through configuration parsing, protocol handling, logging, or
  command-line options.
- Container image or default-configuration issues in [docker/](./docker) and
  [openshift/](./openshift) that expose sensitive services or secrets, or grant
  unsafe privileges in documented deployments.
...
- Bugs requiring local admin/root access on the AMF host with no privilege
  boundary crossed.
...
- Vulnerabilities solely caused by third-party projects or dependencies should
  ...
  the AMF deployment or requires a change in this repository, please report it
  here as well.
- Attacks that require a trusted operator role, such as an already-authorised
  gNB or peer NF deliberately misconfigured by the deployment owner, unless a
...
- Denial of service that only depends on flooding a plaintext, unprotected
  ...
  permitted by the deployment, rather than on an AMF software defect.
